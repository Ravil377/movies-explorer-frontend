    ...
    return (
        <>
            <header className={`header content ${isWhiteHeader ? 'header_white' : ''}`}>
                ...
                <div 
                    className={`burger ${isLogged ? "header__burger_logged" : ""} header__burger ${isMenuOpen ? "burger_active" : ''}`} 
                ...
                <div className={`header__btns ${isLogged ? "header__btns_logged" : ""}`}>
                    <Link to="signup" className="btn btn_nobg header__btn" onClick={onClose}>Регистрация</Link>
                    <Link to="signin" className="btn header__btn" onClick={onClose}>Войти</Link>
                </div>
